(function () {
  ...
  async function backendNews(options = {}) {
    ...
    if (!cache.news) {
      cache.newsRequest ||= getTable("news", null).finally(() => { cache.newsRequest = null; });
  ...
  async function backendSignals(options = {}) {
    ...
    if (!cache.signals) {
      cache.signalsRequest ||= getTable("impact_signals", null).finally(() => { cache.signalsRequest = null; });
    ...
    cache.eventsRequest = Promise.all([backendSignals(options), backendNews(options)])
      .then(([signals, news]) => {
        ...
        const standaloneNews = news
          ...
          .map(normalizeNews);
        const merged = sortByDateDesc([...signalEvents, ...standaloneNews]);
        cache.events = options.limit ? merged.slice(0, normalizeLimit(options.limit)) : merged;
        return cache.events;
      })
  ...
  async function normalizedProjects(options = {}) {
    ...
    if (withEvents && !query && !options.force && cache.normalizedProjectsWithEvents) return cache.normalizedProjectsWithEvents;
  ...
  async function getProjects(query = "", level = "ALL", limit = DEFAULT_LIMIT, options = {}) {
    const events = await normalizedEvents();
    const projects = await normalizedProjects({ query, limit, force: options.force, all: options.all, events });
  ...
  async function ragQuery(query, topK = 5) {
    return request(`${apiBaseUrl()}/rag/query`, {
      method: "POST",
      body: JSON.stringify({ query: normalizeText(query), top_k: topK }),
      timeoutMs: 120000,
    });
  }
  ...
  async function searchSite(query, limit = DEFAULT_LIMIT) {
    ...
    const [projects, news, signals] = settled.map((result) => (result.status === "fulfilled" ? result.value : []));
    return {
