...
function ragSourceToEvent(source) {
    const score = Number(source.risk_score ?? 0);
    return {
      id: source.news_id,
      title: source.title,
      summary: [
        source.risk_category || "",
        source.risk_score != null ? `оценка риска ${source.risk_score}/100` : "",
        `релевантность ${Math.round((source.score || 0) * 100)}%`,
      ].filter(Boolean).join(" · "),
      category: source.risk_category || "Релевантная новость",
      project_name: source.project_name || "Без привязки к ЖК",
      source: source.source || "",
      source_url: source.url || "#",
      published_at: source.date || new Date().toISOString(),
      level: source.level || (score >= 70 ? "RED" : score >= 35 ? "YELLOW" : "GREEN"),
      score,
    };
  }
...
async function askStreamInsight(question) {
    ...
    try {
      // локальная аналитика по потоку (метрики, приоритетные объекты)
      const insight = buildStreamInsight(question);

      // реальный RAG: векторный поиск по базе новостей + ответ DeepSeek
      try {
        const rag = await window.api.ragQuery(question || "Какие риски видны по всем новостям?");
        if (rag) {
          if (rag.answer) {
            insight.lead = rag.answer;
            insight.detailed_analysis = rag.answer;
            insight.verdict = rag.mode === "ai_handler"
              ? "Ответ RAG-сервиса по базе новостей"
              : "Ответ DeepSeek по релевантным новостям";
          } else if (rag.note) {
            insight.lead = `${rag.note} Ниже — релевантные новости, найденные поиском, и локальная аналитика потока.`;
          }
          if (Array.isArray(rag.sources) && rag.sources.length) {
            insight.sources = rag.sources.map(ragSourceToEvent);
          }
        }
      } catch (error) {
        insight.lead = `RAG-бэкенд недоступен (${error.message}). Показана локальная аналитика потока.`;
      }

      state.streamInsight = insight;
