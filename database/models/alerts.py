...
class ImpactSignal(SQLBase):
    ...
    # Категория риска для интерфейса (Срыв сроков / Юридический / Финансовый / Репутация).
    # Заполняется пайплайном; для дампа — эвристикой в scripts/restore_dump.sh.
    risk_category = sql.Column(sql.String, nullable=True)
    news_id = sql.Column(
